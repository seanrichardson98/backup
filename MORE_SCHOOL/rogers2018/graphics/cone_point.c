...
int main() {
    ...
    //ulo, uhi, ustep; v... ; t...
    double ulo = 0; double uhi = 1;   double ustep = 0.0007;
    double vlo = 0; double vhi = 1; double vstep = 0.0007;
    //r given by u(t,u,v);
    ...
    double u,v,t;
    coi[0] = 0.5; coi[1] = 0; coi[2] = 0.5;
    //double r = 5;
    //eye[0] = 5*sin(t); eye[1] = 0; eye[2] = 5*cos(t);
    eye[0] = 0; eye[1] = 0.7; eye[2] = -0.7;
    ...
    for (u = ulo; u <= uhi; u += ustep) { //step through theta
        for (v = vlo; v <= vhi; v += vstep) { //step through phi
            ...
            //normal:
            mirror_edge(a,u+du,v); D3d_mat_mult_pt(a,m,a);
            mirror_edge(b,u,v+dv); D3d_mat_mult_pt(b,m,b);
            subtract_vec(a,a,p);
            subtract_vec(b,b,p);
            x_prod(n,a,b);
            normalize(n);
