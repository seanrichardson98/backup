...
void torus_fl(double p[3], double u, double v) {
    u /= 2*M_PI;
    v /= 2*M_PI;
    p[0] = u-0.5;
    p[2] = v-0.5;
...
void glue_color(double rgb[3], double u, double v) {
    double thick = 0.05;
    if ((fabs(u-1) < thick) || (fabs(u) < thick)) {
       rgb[0] = 1; rgb[1] = 1; rgb[2] = 0; 
    }
    if ((fabs(v-1) < thick) || (fabs(v) < thick)) {
...
int main() {
    ...
    for (u = ulo; u <= uhi; u += ustep) { //step through theta
        for (v = vlo; v <= vhi; v += vstep) { //step through phi
            ...
            //normal:
            torus_fl(a,u+du,v); D3d_mat_mult_pt(a,m,a);
            torus_fl(b,u,v+dv); D3d_mat_mult_pt(b,m,b);
            subtract_vec(a,a,p);
            subtract_vec(b,b,p);
            x_prod(n,a,b);
            normalize(n);
            ...
            glue_color(RGB,u/(2*M_PI),v/(2*M_PI));
