...
int main() {
    ...
    double tlo = phase[0]; double thi = phase[5]; double tstep = 0.1;
    //int k = 6; int l = 6;
    //r given by u(t,u,v2;
    ...
    double u,v,t;
    for (t = thi; t >= tlo; t -= tstep) { //step through time
        i = t/tstep+0.1+880;
        ...
        for (u = ulo; u <= uhi; u += ustep) { //step through theta
            for (v = vlo; v <= vhi; v += vstep) { //step through phi
                ...
                double r_pure = cos(t)*cos(6*u)*cos(6*v);
        ...
        sprintf(fname,"part3R_rush/pic%04d.xwd",i);
